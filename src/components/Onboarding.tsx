        ...
        <AnimatePresence mode="wait">
          ...
          {(step === 1 || step === 2) && (
            <motion.div key="form" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex-1 flex flex-col p-6">
              ...
              {showLicenseReminder && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-start gap-2 bg-primary/10 border border-primary/20 rounded-xl px-3 py-2.5 mb-4">
                  <Key className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" />
                  <p className="text-xs text-primary/90">Keep your license key safe. You'll need it to reactivate your account.</p>
                </motion.div>
              )}
