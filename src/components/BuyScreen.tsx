...
const BuyScreen = ({ initialSymbol, mode: initialMode }: BuyScreenProps) => {
  ...
  const handleExecute = () => {
    ...
    if (mode === "buy") buyToken(selectedSymbol, amount);
    else sellToken(selectedSymbol, tokenAmount);
    setSuccess(true);
    setTimeout(() => { setActiveTab("wallet"); setSuccess(false); setExploreBuySymbol(""); }, 1500);
  };
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="px-4 pb-3">
          <div className="flex items-center gap-2 bg-secondary rounded-xl px-3 py-2.5">
            ...
            <input type="text" placeholder="Search tokens..." value={search} onChange={(e) => setSearch(e.target.value)} autoFocus
      ...
      <div className="flex-1 px-4 space-y-4 pt-2">
        <div className="flex bg-secondary rounded-xl p-1">
          <button onClick={() => setMode("buy")}
            className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${mode === "buy" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Buy</button>
          <button onClick={() => setMode("sell")}
            className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${mode === "sell" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>Sell</button>
        </div>
          ...
          <div className="flex gap-2 mt-2">
            {presets.map(p => (
              <button key={p} onClick={() => setUsdAmount(String(p))}
                className="flex-1 py-2 rounded-xl bg-secondary text-foreground text-xs font-medium hover:bg-muted transition-colors">${p}</button>
            ))}
          </div>
          {amount > 0 && selectedCoin && (
            <p className="text-xs text-muted-foreground mt-2">≈ {tokenAmount.toLocaleString(undefined, { maximumFractionDigits: 6 })} {selectedSymbol}</p>
          )}
        </div>
      </div>
