...
const BuyScreen = () => {
  const { setActiveTab, setExploreBuySymbol, tokens, buyToken } = useWallet();
  ...
  const setAmount = (symbol: string, val: string) => setAmounts(prev => ({ ...prev, [symbol]: val }));
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative">
      <div className="flex items-center gap-3 px-4 pt-3 pb-2">
        <button onClick={() => { setExploreBuySymbol(""); setActiveTab("wallet"); }} className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center text-muted-foreground hover:bg-secondary/80 transition">
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-24">
        {filtered.map(coin => {
          ...
          return (
            <div key={coin.symbol} className={`mb-2 rounded-2xl bg-secondary p-3 transition-all ${wasJustBought ? 'ring-2 ring-success/50 bg-success/5' : ''}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full overflow-hidden bg-muted flex items-center justify-center relative">
                    ...
                    {isBuying && <div className="absolute inset-0 bg-background/80 flex items-center justify-center"><div className="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin" /></div>}
                    {wasJustBought && !isBuying && <div className="absolute inset-0 bg-success/20 flex items-center justify-center rounded-full"><Check className="w-5 h-5 text-success" /></div>}
                  </div>
                  <div>
                ...
                <div className="text-right">
                  ...
                  {currentBalance > 0 && <p className="text-[10px] text-muted-foreground">Owned: {currentBalance.toLocaleString(undefined, { maximumFractionDigits: 4 })}</p>}
                </div>
              </div>
              <div className="mt-2 flex items-center gap-2">
                <div className="relative flex-1">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">$</span>
                  <input type="number" value={usdVal} onChange={(e) => setAmount(coin.symbol, e.target.value)} placeholder="0.00" disabled={isBuying}
                ...
                {tokenAmount > 0 && <p className="text-xs text-muted-foreground whitespace-nowrap">≈ {tokenAmount.toLocaleString(undefined, { maximumFractionDigits: 6 })} {coin.symbol}</p>}
              </div>
            </div>
          );
        })}
      ...
      {totalCost > 0 && (
        <div className="absolute bottom-0 left-0 right-0 px-4 pb-4 pt-2 bg-gradient-to-t from-background via-background to-transparent z-10">
          <button onClick={handleBuyAll} disabled={!!buying}
            className="w-full py-3.5 rounded-2xl bg-primary text-primary-foreground font-semibold text-sm flex items-center justify-center gap-2 hover:bg-primary/90 disabled:opacity-50 transition active:scale-[0.98] shadow-lg shadow-primary/20">
            {buying ? <><div className="w-4 h-4 border-2 border-primary-foreground border-t-transparent rounded-full animate-spin" />Buying...</> :
              <><ShoppingCart className="w-4 h-4" />Buy — ${totalCost.toLocaleString(undefined, { maximumFractionDigits: 2 })}</>}
          </button>
        </div>
      )}
    </div>
